namespace FoodStreetAudioGuide
{
    public class LocalizedText
    {
        public string PageTitle { get; init; } = string.Empty;
        public string SearchHint { get; init; } = string.Empty;
        public string NearbyFilter { get; init; } = string.Empty;
        public string OpenNowFilter { get; init; } = string.Empty;
        public string TopRatedFilter { get; init; } = string.Empty;
        public string Under200Filter { get; init; } = string.Empty;
        public string ExploreTab { get; init; } = string.Empty;
        public string SavedTab { get; init; } = string.Empty;
        public string AwayLabel { get; init; } = string.Empty;
        public string UnavailableContentMessage { get; init; } = string.Empty;
        public string CancelText { get; init; } = string.Empty;
        public string ErrorTitle { get; init; } = string.Empty;
        public string AudioErrorPrefix { get; init; } = string.Empty;
        public string ScriptDialogTitle { get; init; } = string.Empty;
        public string CloseText { get; init; } = string.Empty;
        public string PopupCuisineLabel { get; init; } = string.Empty;
        public string PopupDistanceLabel { get; init; } = string.Empty;
        public string PopupRatingLabel { get; init; } = string.Empty;
        public string PopupReviewsLabel { get; init; } = string.Empty;
        public string PopupHoursLabel { get; init; } = string.Empty;
        public string PopupSpecialtiesLabel { get; init; } = string.Empty;
        public string AudioBadgeText { get; init; } = string.Empty;
        public string AudioReadyLabel { get; init; } = string.Empty;
        public string AudioNotReadyLabel { get; init; } = string.Empty;
        public string DownloadAudioButton { get; init; } = string.Empty;
        public string DownloadingAudioButton { get; init; } = string.Empty;
        public string AudioDownloadedMessage { get; init; } = string.Empty;
        public string AudioDownloadFailedMessage { get; init; } = string.Empty;
        public string DownloadedAudioTitle { get; init; } = string.Empty;
        public string DownloadedAudioEmpty { get; init; } = string.Empty;
        public string DeleteText { get; init; } = string.Empty;
        public string LanguageLabel { get; init; } = string.Empty;
        public string DownloadedAtLabel { get; init; } = string.Empty;

        public string LanguageSelectionTitle { get; init; } = string.Empty;
        public string RecommendedLabel { get; init; } = string.Empty;
        public string ContinueButton { get; init; } = string.Empty;
        public string ChangeAnytimeText { get; init; } = string.Empty;
        public string QrTitle { get; init; } = string.Empty;
        public string QrCameraPermissionMessage { get; init; } = string.Empty;
        public string QrGuideText { get; init; } = string.Empty;
        public string QrInvalidMessage { get; init; } = string.Empty;
        public string QrNotFoundMessage { get; init; } = string.Empty;
        public string MapTitle { get; init; } = string.Empty;
        public string MapNoDataMessage { get; init; } = string.Empty;
        public string MapOpenDetailText { get; init; } = string.Empty;
        public string MapUserLocationText { get; init; } = string.Empty;
        public string MapNearestRoutePrefix { get; init; } = string.Empty;
        public string MapRouteUnavailableText { get; init; } = string.Empty;
        public string ReadMoreText { get; init; } = string.Empty;
        public string ReadLessText { get; init; } = string.Empty;
        public string LoadingNearbyStallsText { get; init; } = string.Empty;
        public string RefreshDataText { get; init; } = string.Empty;
        public string RefreshingDataText { get; init; } = string.Empty;
        public string NavigateToStallText { get; init; } = string.Empty;
        public string RateThisStallText { get; init; } = string.Empty;
        public string RatingConfirmText { get; init; } = string.Empty;
        public string RatingConfirmMessage { get; init; } = string.Empty;
        public string RatingAlreadySubmittedMessage { get; init; } = string.Empty;
        public string RatingSubmittedMessage { get; init; } = string.Empty;
        public string RatingSubmitFailedMessage { get; init; } = string.Empty;
    }
}
